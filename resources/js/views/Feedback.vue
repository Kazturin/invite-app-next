<template>
    ...
    <div class="text-center text-3xl font-vivaldi w-full">
        КБ
    </div>
    <div
        ...
        <div class="flex flex-col justify-between">
            <div>
                <h2 class="text-4xl lg:text-5xl font-bold text-gray-600 leading-tight">Бізге хабарлама қалдырыңыз</h2>
            </div>
        ...
        <div class="">
            <form @submit.prevent="save" class="mt-2 w-full">
                <Alert v-if="errorMsg" class="mb-4">
                    {{ errorMsg }}
                    <span
                        @click="errorMsg = ''"
        ...
        >
          ...
          />
            ...
            <div>
                <span class="uppercase text-sm text-gray-600 font-bold">Аты-жөніңіз</span>
                <input v-model="model.name" class="w-full bg-gray-300 text-gray-900 mt-2 p-3 rounded-lg focus:outline-none focus:shadow-outline"
                       type="text" placeholder="">
            </div>
            <div class="mt-8">
                <span class="uppercase text-sm text-gray-600 font-bold">Email</span>
                <input v-model="model.email" class="w-full bg-gray-300 text-gray-900 mt-2 p-3 rounded-lg focus:outline-none focus:shadow-outline"
                       type="text">
            </div>
            <div class="mt-8">
                <span class="uppercase text-sm text-gray-600 font-bold">Хабарлама</span>
            ...
            <div class="mt-8">
                <button
                    :disabled = "saveLoad"
                    class="disabled:cursor-not-allowed flex items-center disabled:bg-indigo-400 uppercase text-sm font-bold tracking-wide bg-indigo-500 text-gray-100 p-3 rounded-lg w-full focus:outline-none focus:shadow-outline">
                   <span v-if="saveLoad" class="w-5 h-5 mr-2 rounded-full animate-spin
                    border-2 border-solid border-white border-t-transparent"></span><span>Хабарлама жіберу</span>
                </button>
            </div>
            </form>
        </div>
    </div>
        <div class="text-center text-white py-6">&copy; toi-invite.kz</div>
    </div>
</template>
...
let saveLoad = ref(false);

function  save(){
    saveLoad.value = true;
    return axiosClient.post("/feedback", model.value).then(() => {
